...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly create martin-saulis-com-edf0
  fly create martin-saulis-com-edf0-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app martin-saulis-com-edf0
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app martin-saulis-com-edf0-staging
  ```

  If you don't have openssl installed, you can also use [1password](https://1password.com/generate-password) to generate
  a random secret, just replace `$(openssl rand -hex 32)` with the generated secret.

- Create a persistent volume for the sqlite database for both your staging and production environments. Run the
  following:

  ```sh
  fly volumes create data --size 1 --app martin-saulis-com-edf0
  fly volumes create data --size 1 --app martin-saulis-com-edf0-staging
  ```
